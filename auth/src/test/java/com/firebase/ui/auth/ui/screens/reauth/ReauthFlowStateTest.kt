...
class ReauthFlowStateTest {
    ...
    private fun ReauthFlowState.armed(
        resolver: CompletableDeferred<Boolean>? = null,
    ): AuthState.Reauthentication.Request {
        val request = request(resolver)
        arm(AuthState.Reauthentication.Required(request))
        return request
    }

    /**
     * The counter `addReauthenticationDrainer` kept is gone because this is the same question:
     * with nothing armed there is no conversation for a provider state to belong to, so it stays
     * the host's.
     */
    @Test
    fun `nothing is folded while nothing is armed`() {
        val holder = holder()
